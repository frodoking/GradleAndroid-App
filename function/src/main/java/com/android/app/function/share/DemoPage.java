...
import com.android.app.function.onekeyshare.OnekeyShare;
import com.android.app.function.onekeyshare.OnekeyShareTheme;
import com.android.app.function.onekeyshare.Shake2Share;
import com.android.app.function.onekeyshare.ShareCore;
...
public class DemoPage extends SlidingMenuPage implements
	...
	private void showShare(boolean silent, String platform, boolean captureView) {
		...
		} else if (ShareActivity.TEST_TEXT != null && ShareActivity.TEST_TEXT.containsKey(0)) {
			oks.setText(ShareActivity.TEST_TEXT.get(0));
		} else {
		...
		if (captureView) {
			...
			oks.setImagePath(CustomShareFieldsPage.getString("imagePath", ShareActivity.TEST_IMAGE));
			oks.setImageUrl(CustomShareFieldsPage.getString("imageUrl", ShareActivity.TEST_IMAGE_URL));
		...
		oks.setFilePath(CustomShareFieldsPage.getString("filePath", ShareActivity.TEST_IMAGE));
		oks.setComment(CustomShareFieldsPage.getString("comment", context.getString(R.string.share)));
		oks.setSite(CustomShareFieldsPage.getString("site", context.getString(R.string.share)));
...
//		oks.addHiddenPlatform(TencentWeibo.NAME);
	...
	public void onClick(View v) {
        ...
        int i = v.getId();
        if (i == R.id.btnShareAllGui) {
            showShare(false, null, false);

        } else if (i == R.id.btnShareAll) {
            showShare(true, null, false);

        } else if (i == R.id.btnShareView) {
            Shake2Share ss = new Shake2Share();
            ss.setOnShakeListener(new Shake2Share.OnShakeListener() {
                public void onShake() {
        ...
        } else if (i == R.id.btnFlSw) {
            Platform plat = ShareSDK.getPlatform("SinaWeibo");
            plat.setPlatformActionListener(this);
            plat.followFriend(ShareAdapter.SDK_SINAWEIBO_UID);

        } else if (i == R.id.btnFlTc) {
            Platform plat = ShareSDK.getPlatform("TencentWeibo");
            plat.setPlatformActionListener(this);
            plat.followFriend(ShareAdapter.SDK_TENCENTWEIBO_UID);

        } else if (i == R.id.btnGetToken) {
            GetTokenPage page = new GetTokenPage();
            page.show(getContext(), null);

        } else if (i == R.id.btnGetInfor) {
            GetInforPage page = new GetInforPage();
            page.setType(0);
            page.show(getContext(), null);

        } else if (i == R.id.btnGetUserInfor) {
            GetInforPage page = new GetInforPage();
            page.setType(1);
            page.show(getContext(), null);
